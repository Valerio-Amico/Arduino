...
class ArduinoUno:
    ...
    def read_distance(self, trig_pin:int, echo_pin:int) -> str:
        """
        works with arduino module HC-SR04. check specs online.
        args:
            trig_pin : arduino pin connected to trig pin of HC-SR04
            echo_pin : arduino pin connected to echo pin of HC-SR04
        """
        self.write_string(f"L{trig_pin}_{echo_pin}:?")
        self.serial_com.flush()
        return self.read()

def main():
    arduino = ArduinoUno("692CE576A1A2CCF8C364")
    while True:
        val = arduino.read()
        print(val)
        time.sleep(1)
